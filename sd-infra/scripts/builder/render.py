...
# ── 布局常量（被 node_height / node_width / render 主函数共用）──────────────────
...
START_X             = 60
START_Y             = 60
MAX_GROUP_ROW_SPAN  = 3     # group 跨行超过此值时不画容器框
...
def _export_view_yaml(arch: dict, positions: dict, groups: list, path: Path):
    """将自动布局生成的坐标导出为 view.yaml，供下次幂等还原使用。"""
    nodes_view = {}
    for n in arch.get("nodes", []):
        nid = n["id"]
        if nid in positions:
            x, y, w, h = positions[nid]
            nodes_view[nid] = {"x": round(x, 1), "y": round(y, 1),
    ...
    for g in groups:
        ...
        groups_view[gid_] = {"x": round(cx, 1), "y": round(cy, 1),
                             "w": round(cw, 1), "h": round(ch, 1)}

    view = {
        "meta": {
            "arch": arch.get("meta", {}).get("title", "unknown") + ".yaml",
            "version": "1.0",
...
{cells_xml}{legend}      </root>
    ...
    # ── 自动生成 view.yaml（幂等还原用）───────────────────────────────────────
    # 仅在自动布局（非 view.yaml 输入）时导出，避免覆盖用户手调坐标
    if not use_view:
        view_yaml_path = Path(output_path).with_suffix(".view.yaml")
        _export_view_yaml(arch, positions, groups, view_yaml_path)
        print(f"✓ {view_yaml_path} (auto-generated for idempotent restore)")
