...
@dataclass
class ScalePathResult:
    current_tier_name: str
    next_tier_name: str
    trigger_condition: str
    trigger_daily_events: str
    trigger_total_events: str
...
class CalcResult:
    ...
    # 数据节点数自动推导（标准集群）
    recommended_data_nodes: int
    data_nodes_auto_derived: bool

    # 扩容路径
    scale_path: Optional[ScalePathResult] = None
...
def derive_data_nodes(planned_daily_wan: float, total_yi: float, cdp_mode: str) -> int:
    """
    标准集群数据节点数自动推导。
    按规划日事件量和总存储量，从标准集群档位中匹配推荐数据节点数。
    非标准集群返回 0（不适用）。
    """
    if cdp_mode != "standard":
        return 0

    for (max_daily_wan, max_total_yi, name, mode, meta_n, meta_cpu, meta_mem, data_n, data_cpu, data_mem) in CDP_TIERS:
        if mode != "standard":
            continue
        if planned_daily_wan <= max_daily_wan and total_yi <= max_total_yi:
            return data_n

    # 超出所有预设档位，按每节点承载 3亿/天 估算
    return max(6, math.ceil(planned_daily_wan / 30000))


def find_next_tier(current_tier_name: str, planned_daily_wan: float, total_yi: float) -> Optional[tuple]:
    """
    查找当前档位的下一档。
    返回：(next_tier_name, trigger_daily_wan, trigger_total_yi) 或 None（已是最高档）
    """
    # 找到当前档位在列表中的索引
    current_idx = None
    for i, tier in enumerate(CDP_TIERS):
        if tier[2] == current_tier_name:
            current_idx = i
            break
    ...
    # 从当前档位之后查找第一个同部署模式或更高规模的档位
    current_mode = CDP_TIERS[current_idx][3]

    for j in range(current_idx + 1, len(CDP_TIERS)):
        next_tier = CDP_TIERS[j]
        # 单机→Mini→Standard 跨模式也视为升级路径
        return (next_tier[2], next_tier[0], next_tier[1])
...
def calc_scale_path(
    cdp_tier: CDPTierResult,
    planned_daily_wan: float,
    total_yi: float,
) -> Optional[ScalePathResult]:
    """
    计算扩容路径：当前档位 → 下一档位的触发条件。
    """
    next_info = find_next_tier(cdp_tier.tier_name, planned_daily_wan, total_yi)
    if not next_info:
        return None

    next_name, trigger_daily, trigger_total = next_info

    # 触发条件：日事件量或总存储量先达到哪个阈值
    daily_gap = max(0, trigger_daily - planned_daily_wan)
    total_gap = max(0, trigger_total - total_yi)

    if daily_gap <= 0 or total_gap <= 0:
        trigger_condition = "已接近或达到扩容阈值，建议提前规划"
    else:
        trigger_condition = (
            f"日事件量达到 {wan_to_yi(trigger_daily)} 或 "
            f"总存储量达到 {trigger_total:.0f}亿 时升级至 {next_name}"
        )

    return ScalePathResult(
        current_tier_name=cdp_tier.tier_name,
        next_tier_name=next_name,
        trigger_condition=trigger_condition,
        trigger_daily_events=wan_to_yi(trigger_daily),
        trigger_total_events=f"{trigger_total:.0f}亿",
    )


def apply_physical_bump(cdp_tier: CDPTierResult) -> CDPTierResult:
    """
    物理机模式：配置整体上浮一档。
    规则：
      - 单机/Mini：CPU 和内存各升一档（如 8C→16C，64G→128G）
      - 标准集群：元数据节点和数据节点的 CPU 各升一档
      - 磁盘：顺序盘单块规格向上取整到下一标准盘型（如 500G→1000G）
      - 物理机无横向扩容能力，需预留更多余量
    """
    def bump_cpu(cpu_str: str) -> str:
        val = int(cpu_str.replace("C", "").replace("+", "").strip())
        # 8→16, 16→32, 32→64
        if val <= 8:
            return "16C"
        elif val <= 16:
    ...
    def bump_mem(mem_str: str) -> str:
        val = int(mem_str.replace("G", "").replace("+", "").strip())
    ...
    new_meta_cpu = bump_cpu(cdp_tier.meta_cpu) if cdp_tier.meta_cpu != "-" else "-"
    new_meta_mem = bump_mem(cdp_tier.meta_mem) if cdp_tier.meta_mem != "-" else "-"
    new_data_cpu = bump_cpu(cdp_tier.data_cpu)
    new_data_mem = bump_mem(cdp_tier.data_mem)

    return CDPTierResult(
        tier_name=cdp_tier.tier_name + "（物理机）",
        deploy_mode=cdp_tier.deploy_mode,
        meta_cpu=new_meta_cpu,
        meta_mem=new_meta_mem,
        meta_nodes=cdp_tier.meta_nodes,
        data_cpu=new_data_cpu,
        data_mem=new_data_mem,
        data_nodes=cdp_tier.data_nodes,
        reason=cdp_tier.reason + "；物理机配置上浮一档（不便扩容，预留余量）",
    )


def bump_disk_for_physical(disk_result: DiskResult) -> DiskResult:
    """物理机模式：顺序盘单块规格向上取整到下一标准盘型。"""
    current = disk_result.per_disk_recommended_gb
    # 找到下一个更大的标准盘型
    next_size = None
    for s in STANDARD_DISK_SIZES_GB:
        if s > current:
    ...
    if next_size is None:
        # 已超出最大标准盘，按 1000G 步进
        next_size = math.ceil(current / 1000) * 1000

    return DiskResult(
        total_event_storage_gb=disk_result.total_event_storage_gb,
        total_seq_disk_gb=disk_result.total_seq_disk_gb,
        per_node_seq_disk_gb=disk_result.per_node_seq_disk_gb,
        per_disk_raw_gb=disk_result.per_disk_raw_gb,
        per_disk_recommended_gb=next_size,
        disks_per_node=disk_result.disks_per_node,
        data_nodes=disk_result.data_nodes,
        arm_doubled=disk_result.arm_doubled,
    )
...
def run_calc(
    ...
    # CDP 选型（先执行，用于推导数据节点数）
    cdp_tier = select_cdp_tier(daily_wan, retention_days, history_wan, growth_multiplier, dau_wan)

    # 物理机模式：配置整体上浮一档
    is_physical = arch.lower() == "physical"
    if is_physical:
        cdp_tier = apply_physical_bump(cdp_tier)

    # 数据节点数自动推导（标准集群）
    planned_daily = daily_wan * growth_multiplier
    total_yi = (daily_wan * retention_days + history_wan) / 10000
    recommended_data_nodes = derive_data_nodes(planned_daily, total_yi, cdp_tier.deploy_mode)

    # 如果用户未显式指定 data_nodes（使用默认值 3）且是标准集群，使用自动推导值
    data_nodes_auto_derived = False
    if cdp_tier.deploy_mode == "standard" and data_nodes == 3:
        # 只有当推导值与默认值不同时才自动覆盖
        if recommended_data_nodes != data_nodes:
            data_nodes = recommended_data_nodes
            data_nodes_auto_derived = True
            inputs["data_nodes"] = data_nodes
            inputs["data_nodes_auto_derived"] = True
    ...
    # 物理机模式：磁盘上浮一档
    if is_physical:
        disk = bump_disk_for_physical(disk)
    ...
    # 扩容路径计算（物理机模式不输出扩容路径，因为物理机无横向扩容能力）
    scale_path = None
    if not is_physical:
        scale_path = calc_scale_path(cdp_tier, planned_daily, total_yi)
    ...
    if is_physical:
        warnings.append("⚠️  物理机模式：配置已整体上浮一档，磁盘规格已取下一标准盘型。物理机无横向扩容能力，请确保初始配置留足余量。")
    if sf_tier and not sf_tier.matched:
        warnings.append("⚠️  SF 规模超出预设档位，需联系 SaaS 运维单独评估。")
    if data_nodes_auto_derived:
        warnings.append(
            f"⚠️  数据节点数已由脚本自动推导为 {recommended_data_nodes} "
            f"（基于规划日事件量 {wan_to_yi(planned_daily)} 和总存储量 {total_yi:.0f}亿）。"
            "如客户有特定要求，可通过 --data-nodes 显式覆盖。"
        )
    ...
    return CalcResult(
        ...
        recommended_data_nodes=recommended_data_nodes,
        data_nodes_auto_derived=data_nodes_auto_derived,
...
def format_result(r: CalcResult) -> str:
    ...
    if r.data_nodes_auto_derived:
        lines.append(f"\n  ⚠️ 数据节点数自动推导: {r.recommended_data_nodes} 台")
        lines.append(f"     （基于规划日事件量和总存储量，原默认值 3 已被覆盖）")
    ...
    if r.scale_path:
        lines.append("\n【扩容路径】")
        sp = r.scale_path
        lines.append(f"  当前档位 : {sp.current_tier_name}")
        lines.append(f"  下一档位 : {sp.next_tier_name}")
        lines.append(f"  触发条件 : {sp.trigger_condition}")
        lines.append(f"  触发阈值 : 日事件量 {sp.trigger_daily_events} / 总存储 {sp.trigger_total_events}")
...
def main():
    ...
    parser.add_argument("--arch",              default="x86",  help="CPU 架构：x86 / arm / physical（默认 x86）")
