...
| 变量 | 说明 | 获取方式 |
|------|------|----------|
| `SA_HOST` | 神策 CDP 地址 | 客户环境 URL，如 `https://demo.sensorsdata.cn` |
| `SA_PROJECT` | 项目 ID | 登录后 URL 中 `project=` 的值 |
| `SA_TOKEN` | HTTP API token | 神策后台 → 数据接入 → 获取 token |
...
### Phase 2 完成后（客户确认埋点方案后）

在 `.env` 中补充：

| 变量 | 说明 |
|------|------|
| `TRACKING_PLAN_PATH` | 埋点方案 Excel 的绝对路径或相对路径 |
