...
def _extract_enum_values(
    ...
    Configurable limits (default expanded from 60/20 to 100/100):
      max_item_len — max length of a single enum value
      max_count    — max number of values to accept
      min_count    — min number of values to accept
    """
    if not text:
        return None
    ...
    if "；" in text:
        parts = text.split("；")
        values = [p.strip().strip('"').strip('"').strip('"').strip() for p in parts]
        values = [v for v in values if v and len(v) < max_item_len]
    ...
    quoted = re.findall(r'["""]([^"""]{1,%d})["""]' % max_item_len, text)
    if len(quoted) >= min_count:
        return quoted[:max_count]
...
class TrackingPlan:
    ...
    # Supports both MP format (Custom Event / Preset Event / Public Property / User Attribute)
    # and standard format (Events / Details(Event) / Users).
    SHEET_MAPPINGS = {
    ...
    # Column-header mappings for standard format vs MP format.
    # Keys are canonical column identifiers; values are lists of possible header strings.
    COLUMN_MAPPINGS = {
        "event_var_name": [
            "event variable name",
    ...
    def _find_column(self, headers: tuple, canonical: str) -> Optional[int]:
        """Return the 0-based column index for a canonical column identifier."""
        candidates = self.COLUMN_MAPPINGS.get(canonical, [canonical])
        for idx, h in enumerate(headers):
            if h is None:
                continue
            h_norm = re.sub(r"[\s（）()]+", " ", str(h).strip().lower()).strip()
            for cand in candidates:
                if h_norm == cand or h_norm.startswith(cand):
                    return idx
        return None
    ...
    def has_mp_events(self) -> bool:
        ...
        or other non-$MP preset events that are managed by the platform (e.g. $WebClick).
        """
        preset_prefixes = ("$MP", "$Web")
        return any(
            any(name.startswith(p) for p in preset_prefixes)
