...
所有数据扫描、统计、对比操作**必须使用已封装脚本**：
- 快速扫描 JSONL → `scan_jsonl.py`
...
**严禁在聊天中编写并执行 10 行以上的临时 Python 代码。** 只有脚本不存在且任务是一次性 3-5 行辅助代码时才允许例外。

### 规则 1：先确认校验意图，再读文件

用户输入 `/sd-validate-data` 后，AI 必须**先展示选项并等待选择**，不要立即全面扫描：
